...
% @email        kamil@utdallas.edu
% @affiliation  School of Electrical Engineering, University of Texas at Dallas
% @brief        Speech spectrogram plot function
%______________________________________________________________________________________________________________________
%
% @inputs       speech      - time domain speech signal vector
%               fs          - sampling frequency (Hz), f.e. 8000, optionally include upper frequency cutoff for the plot
%               T           - vector of frame width, Tw, and frame shift, Ts, in milliseconds, i.e. [Tw, Ts]
%               w           - analysis window handle, f.e. @hanning 
...
%               annotations - how to annotate the x-axis, default: 'time', or specify a phone or word file in the following format:
%
%                                i) each line describes a single lingustic/phonetic unit
%                               ii) first column contains the start sample indexes
%                              iii) second column contains the end sample indexes
%                               iv) third column contains the corresponding lingustic/phonetic labels
%
%                            e.g. see timit_test_dr1_mdab0_sa1.wrd:
%
%                                2320 4730 she
%                                4730 6866 had
%                                6866 8520 your
...
%______________________________________________________________________________________________________________________
%
% @usage        [ handle ] = myspectrogram( speech, fs, T, w, nfft, Slim, alpha, cmap, cbar, type, annotations );
%
% @examples     myspectrogram( 'timit_test_dr1_mdab0_sa1.wav' );
%               myspectrogram( speech, 16000 );
%               myspectrogram( speech, [16000 5500], [18 1], @hanning, 2048, [-61 -1], false, 'default', false, 'lp', 'timit_test_dr1_mdab0_sa1.phn' );
%               myspectrogram( speech, [16000 8000], [18 1], @hamming, 1024, [-45 -2], [1 -0.97], 'default', true, 'per', 'timit_test_dr1_mdab0_sa1.wrd' );
%
%______________________________________________________________________________________________________________________
function [ handle ] = myspectrogram( s, fs, T, w, nfft, Slim, alpha, cmap, cbar, type, annotations )

    %__________________________________________________________________________________________________________________
    % VALIDATE INPUTS, SET DEFAULTS
    switch nargin
    case 1, annotations='time'; type='per'; cbar=false; cmap='default'; alpha=false; Slim=[-61,-1]; nfft=2048; w=@hanning; T=[18,1]; fs=8000;
    case 2, annotations='time'; type='per'; cbar=false; cmap='default'; alpha=false; Slim=[-61,-1]; nfft=2048; w=@hanning; T=[18,1];  
    case 3, annotations='time'; type='per'; cbar=false; cmap='default'; alpha=false; Slim=[-61,-1]; nfft=2048; w=@hanning; 
    case 4, annotations='time'; type='per'; cbar=false; cmap='default'; alpha=false; Slim=[-61,-1]; nfft=2048; 
    case 5, annotations='time'; type='per'; cbar=false; cmap='default'; alpha=false; Slim=[-61,-1]; 
    case 6, annotations='time'; type='per'; cbar=false; cmap='default'; alpha=false;
    case 7, annotations='time'; type='per'; cbar=false; cmap='default'; 
    case 8, annotations='time'; type='per'; cbar=false; 
    case 9, annotations='time'; type='per'; 
    case 10, annotations='time'; 
    case 11
...
%    S(S<eps) = eps;                     % prevent zero spectral magnitude values 
    ...
    %set(gca, 'ytick', [0:2000:48000], 'yticklabel', [0:2:48]); ylim([ 0 fs/2 ]);
    set(gca, 'ytick', [0:1000:48000], 'yticklabel', [0:1:48]); ylim([ 0 5500 ]);
    ylabel('Frequency (kHz)', 'fontsize', 10 );


    [ annotations, labels, label_ticks, xticks, list ] = read_annotations( annotations );

    if( isstr(annotations) )
        switch( lower(annotations) )
        case 'time'
    
            timeticks = [0.5:0.5:10];
            line( repmat(timeticks,2,1), [0 1].'*300, 'color', 'k' );
    
            minorticks = [0.1:0.1:10];
            line( repmat(minorticks,2,1), [0 1].'*150, 'color', 'k' );
    
            text( 0, -400, sprintf('%0.0f',0), 'fontsize', 10 );
            for t = 1:length( timeticks )
                timetick = timeticks(t);
                text( timetick, -400, sprintf('%0.1f',timetick), 'fontsize', 10, 'HorizontalAlignment', 'center' );
            end 
            text( 0.47*(time(end)-time(1)), -1050, sprintf('Time (s)'), 'fontsize', 10 );
        end
    else
        ...
        for t = 1:length( xticks );
            xtick = xticks(t)/fs;
            text( xticks(t)/fs, -350, '|', 'fontsize', 14, 'HorizontalAlignment', 'center', 'color', [1 1 1]*0.75 );
        end

        for l = 1:length( labels )
            label = labels{l};
            text( label_ticks(l)/fs, -400, label, 'fontsize', 7, 'HorizontalAlignment', 'center' );
        end 
    end
...
% @author       Kamil Wojcicki
% @date         Apr 2011
% @brief        Read plot annotations from file
%______________________________________________________________________________________________________________________
function [ annotations, labels, label_ticks, xticks, list ] = read_annotations( filename );

    if( isempty(filename) || strcmp(filename,'') || strcmp(filename,'time') || ~exist(filename,'file') )
        annotations = 'time'; labels = []; label_ticks = []; xticks = []; list = []; return;
    end

    delimiters = sprintf( '%c%c',  ' ', char(9) );
    list = file2cell( filename );
    L = length( list );
    ...
    for l = 1:L
        ...
        if( ~isempty(item) )
            [ start_sample, item ] = strtok( item );
            [ end_sample, lingustic_unit ] = strtok( item, delimiters );
    
            switch( lower(strtrim(lingustic_unit)) )
            case { 'h#', 'ignore me' } % ignore these labels...
            otherwise
                i = i+1;    
    
                annotations(i).start_sample = floor( str2num(start_sample) );
                annotations(i).end_sample = floor( str2num(end_sample) );
                annotations(i).lingustic_unit = strtrim( lingustic_unit );
    
                range = [ annotations(i).start_sample annotations(i).end_sample ];
                annotations(i).label_position = mean(range);
    
                xticks = [ xticks range ];
                labels = [ labels { annotations(i).lingustic_unit } ];
                label_ticks = [ label_ticks annotations(i).label_position ];
            end
        end
    end
...
% Date:         April 2006
% Revision:     001
% Purpouse:     Reads in contents of an ascii file into a cell array. 
%               Each element of the cell array contains one line of the ascii file.
%
%______________________________________________________________________________________________________________________
%
% Inputs:       filename - filename of the input ascii file
%
% Output:       list     - cell array of line strings from input ascii file
%
% Usage:        [list] = file2cell(filename)
%                
%______________________________________________________________________________________________________________________
function [list] = file2cell(filename)
    ...
    %%% FREE FILE HANDLES UPON COMPLETION %%%
    fclose(fid);
