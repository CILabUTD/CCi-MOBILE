...
% Turn the lamp on
app.CustomSignalProcessingApp.Visible = 'on';
app.CustomSignalProcessingApp.Interruptible = 'on';
app.StreamLamp.Interruptible = 'on';
app.StreamLamp.Enable = 'on';
app.StreamLamp.Color = [0.43,0.70,0.24];
pause(0.0001)
...
while frame_no<nframes % use while else timing won't be right
    ...
    if Wait(s)>= 512
        ...
        if (isfield(stimuli,'Left') ==1)&&(p.General.LeftOn == 1)
        ...
        if (isfield(stimuli,'Right') ==1)&&(p.General.RightOn == 1)
...
app.CustomSignalProcessingApp.Interruptible = 'on';
app.StreamLamp.Interruptible = 'on';
app.StreamLamp.Color = [0.76,0.18,0.18];
app.StreamLamp.Enable = 'off';
pause(0.0001)
